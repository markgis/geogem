...
RSpec.describe RadialThin do
  describe 'distance' do
    it 'returns distance from two points' do
      expect(RadialThin.new.distance(400000,200000,400100,200000)).to eq(100)
    end
  end

  describe 'node_split' do
    it 'returns nodes split as array of objects' do
      expect(RadialThin.new.node_split('0 1, 0 2, 0 4, 1 4, 2 4, 4 4')).to eq([{:x=>0, :y=>2}, {:x=>0, :y=>4}, {:x=>1, :y=>4}, {:x=>2, :y=>4}])
    end

  describe'node_distance' do
    it 'returns distance between nodes' do
      expect(RadialThin.new.node_distance('0 1, 0 2, 0 4, 1 4, 2 4, 4 4')).to eq([{:point=>{:x=>0, :y=>2}, :dist_to_next=>2.0}, {:point=>{:x=>0, :y=>4}, :dist_to_next=>1.0}, {:point=>{:x=>1, :y=>4}, :dist_to_next=>1.0}, {:point=>{:x=>2, :y=>4}, :dist_to_next=>-1}])
    end
  end

  describe 'first_node' do
    it 'returns the first node x and y from string' do
      expect(RadialThin.new.first_node('0 1, 0 2, 0 4, 1 4, 2 4, 4 4')).to eq(:point => {:x=>"0", :y=>"1"})
    end
  end

  describe 'last_node' do
    it 'returns the last node x and y from string' do
      expect(RadialThin.new.last_node('0 1, 0 2, 0 4, 1 4, 2 4, 4 4')).to eq(:point => {:x=>"4", :y=>"4"})
    end
  end
